...
// ── Media ─────────────────────────────────────────────────────────────────────
// Each item: { preview, type, isCover, isExisting, dbUrl?, mediaType?, file? }
const mediaList = ref([])
...
const PHOTO_MAX      = 5
const VIDEO_MAX      = 1
const PHOTO_SIZE_MAX = 4 * 1024 * 1024
const VIDEO_SIZE_MAX = 20 * 1024 * 1024
...
function handleFileInput(event) {
  ...
  for (const file of files) {
    ...
    if (isImage && file.size > PHOTO_SIZE_MAX) { errorMsg.value = `Foto "${file.name}" melebihi 4 MB.`; continue }
    if (isVideo && file.size > VIDEO_SIZE_MAX)  { errorMsg.value = `Video "${file.name}" melebihi 20 MB.`; continue }
    if (isImage && (photoCount.value + imageQueue.length) >= PHOTO_MAX) { errorMsg.value = `Maksimal ${PHOTO_MAX} foto.`; continue }
    if (isVideo && videoCount.value >= VIDEO_MAX)  { errorMsg.value = `Maksimal ${VIDEO_MAX} video.`; continue }
    if (isImage) {
...
function openViewer(index) { viewerIndex.value = index; viewerOpen.value = true }
...
async function saveEdits() {
  ...
  try {
    ...
    const newSlug   = generateSlug(form.title.trim(), productId)
    ...
    // 2. Wipe old media records (re-insert with updated is_primary flags)
    ...
    // 3. Re-insert all surviving media
...
async function deleteProduct() {
  ...
  try {
    ...
    // Delete all chats (and cascaded messages/offers) for this product
