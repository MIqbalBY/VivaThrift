...
useSeoMeta({ title: 'Pengikut — VivaThrift' })
...
async function fetchFollowers() {
  ...
  const { data: followerData } = await supabase
    .from('follows')
    .select('follower_id, users!follows_follower_id_fkey ( id, name, username, nrp, faculty, department, avatar_url, gender )')
    .eq('following_id', user.value.id)
    .order('created_at', { ascending: false })

  followers.value = (followerData ?? []).map(f => f.users).filter(Boolean)

  // Fetch who I follow so we can show "follow back" state
...
<template>
  <div class="w-full max-w-3xl mx-auto px-4 md:px-8 py-6">
    ...
    <h1 :ref="reveal" class="text-2xl font-bold mb-6" :class="isDark ? 'text-white' : 'text-[#1e3a8a]'">
      Pengikut
      <span v-if="!loading" class="text-base font-normal" :class="isDark ? 'text-gray-400' : 'text-gray-500'">({{ followers.length }})</span>
    </h1>

    <!-- Loading skeleton -->
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 5" :key="i" class="flex items-center gap-4 p-4 rounded-xl animate-pulse" :class="isDark ? 'bg-slate-800/60' : 'bg-white/60'">
        <div class="w-12 h-12 rounded-full shrink-0" :class="isDark ? 'bg-slate-700' : 'bg-gray-200'" />
        <div class="flex-1 space-y-2">
          <div class="h-4 rounded-full w-1/3" :class="isDark ? 'bg-slate-700' : 'bg-gray-200'" />
          <div class="h-3 rounded-full w-1/2" :class="isDark ? 'bg-slate-700/60' : 'bg-gray-100'" />
        </div>
      </div>
    </div>

    <!-- Empty -->
    <div v-else-if="!followers.length" class="flex flex-col items-center text-center py-20 gap-3">
      <img src="/img/illustrations/empty-cart.svg" alt="Belum ada pengikut" width="192" height="192" loading="lazy" class="w-48 h-auto opacity-80" />
      <p class="font-semibold text-lg" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Belum ada pengikut</p>
      <p class="text-sm" :class="isDark ? 'text-gray-500' : 'text-gray-400'">Mulai jual barang untuk menarik pengikut!</p>
    </div>
    ...
    <div v-else class="space-y-3" :ref="reveal">
      ...
      >
        ...
        >
          {{ followingSet.has(u.id) ? '✓ Mengikuti' : 'Ikuti Balik' }}
        </button>
      </div>
    </div>
  </div>
