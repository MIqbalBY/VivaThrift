...
function onReviewSubmitted() {
  if (reviewTarget.value) markReviewed(reviewTarget.value.orderItemId)
...
<template>
  <div class="w-full max-w-3xl mx-auto px-4 py-8">
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      >
        ...
        <div class="px-4 pb-4 flex flex-wrap gap-2">
          ...
          <!-- Buyer: per-item "Beri Ulasan" / "Sudah Diulas" when order is completed -->
          <template v-if="role === 'buyer' && order.status === 'completed'">
            <template v-for="item in order.order_items" :key="item.id">
              <button
                v-if="!isReviewed(item.id)"
    ...
    <ReviewModal
      v-if="reviewTarget"
      :order-item-id="reviewTarget.orderItemId"
