...
<template>
  ...
  >
    ...
    <div class="vt-hero-enter vt-hero-enter-d2 relative z-10 w-full max-w-sm mx-4 rounded-2xl px-10 py-10" style="background: rgba(255,255,255,0.15); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 8px 32px rgba(0,0,0,0.25);">
      ...
      <!-- Illustration -->
      <img src="/img/illustrations/secure-login.svg" alt="" width="128" height="128" loading="lazy" class="w-32 h-auto mx-auto mb-4 opacity-80" aria-hidden="true" />
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <!-- Forgot password -->
        <NuxtLink to="/auth/forgot-password" class="block text-sm text-white/90 hover:text-white hover:underline -mt-2 transition-colors">Lupa password?</NuxtLink>
        ...
        <div v-if="signupSuccess" class="flex items-start gap-2.5 border border-green-200 rounded-xl px-4 py-3 -mt-1" style="background: linear-gradient(to right, #f0fdf4, #dcfce7, #bbf7d0);">
          ...
          <p class="text-sm text-green-700 leading-snug">
            Akun berhasil dibuat. Cek email kamu lalu verifikasi dulu sebelum masuk.
          </p>
        </div>
    ...
    <!-- Daftar -->
    <p class="vt-hero-enter vt-hero-enter-d3 relative z-10 mt-4 text-sm text-white/80">
      Belum punya akun?
