...
const productCategoryHint = computed(() =>
  [offer.value?.product?.categories?.name, offer.value?.product?.title]
    .filter(Boolean)
    .join(' ')
)

const shippingInsuranceEligible = computed(() =>
  shippingMethod.value === 'shipping' && isShippingInsuranceEligible(productCategoryHint.value)
)
    ...
    <template v-else>
        ...
        <div class="pt-4 space-y-2 text-sm" :class="isDark ? 'border-t border-white/10' : 'border-t border-gray-100'">
          ...
          <div v-if="shippingMethod === 'shipping'" class="flex justify-between">
            ...
            <span class="font-medium" :class="selectedRate ? (isDark ? 'text-green-400' : 'text-green-600') : (isDark ? 'text-gray-500' : 'text-gray-400')">
              {{ selectedRate ? `Rp ${selectedCourierPrice.toLocaleString('id-ID')}` : '—' }}
            </span>
          </div>
          <div v-if="shippingMethod === 'shipping' && shippingInsuranceFee > 0" class="flex justify-between">
            <span :class="isDark ? 'text-gray-400' : 'text-gray-500'">Asuransi Pengiriman</span>
      ...
      >
        ...
        <div v-if="shippingMethod === 'shipping'" class="mt-4">
          ...
          <p class="text-[11px] mb-2" :class="isDark ? 'text-gray-500' : 'text-gray-400'">Daftar kurir diurutkan dari harga termurah dan bisa difilter per tipe layanan.</p>
          ...
          <p v-if="ratesLoading" class="text-xs mt-2" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
            Sedang mencari layanan kurir yang tersedia...
          </p>
          ...
          <div v-if="rates.length" class="mt-4">
            ...
            <p v-if="!visibleRates.length" class="text-xs mt-3" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
            ...
            <div v-if="selectedRate" class="mt-4 space-y-4 rounded-2xl border p-4"
              ...
              <div>
                ...
                <p class="text-[11px] mt-2" :class="isDark ? 'text-gray-500' : 'text-gray-400'">
                  Pilihan aktif: {{ selectedCollectionLabel }}
                </p>
              </div>
              ...
              <div v-if="shippingInsuranceEligible" class="rounded-xl border px-3 py-3"
                ...
                <label class="flex items-start gap-3 cursor-pointer">
                  ...
                  <span class="flex-1 min-w-0">
                    ...
                    <span class="block text-[11px] mt-1" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
                      Cocok untuk elektronik atau gadget. Biaya asuransi 0,5% dari nilai barang.
                    </span>
                  </span>
