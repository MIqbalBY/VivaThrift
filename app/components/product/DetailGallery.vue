...
<template>
  ...
  <div class="relative overflow-hidden rounded-2xl shadow bg-gray-50 group">
    ...
    <template v-if="media.length > 1">
      <button @click.stop="prev" class="vt-img-nav-btn absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition opacity-0 group-hover:opacity-100 z-10">
      ...
      <button @click.stop="next" class="vt-img-nav-btn absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition opacity-0 group-hover:opacity-100 z-10">
    ...
    <!-- Expand -->
    <button v-if="!activeMedia?.isVideo" @click.stop="emit('open-lightbox')" class="vt-img-nav-btn absolute bottom-2 right-2 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition opacity-0 group-hover:opacity-100 z-10">
